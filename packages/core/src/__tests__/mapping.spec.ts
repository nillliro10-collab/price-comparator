...
import { StoreMappingProfile } from '../mapping/mapping.types';
...
describe('MappingService', () => {
  ...
  const mockProfile: StoreMappingProfile = {
    name: 'Test Store',
  ...
  it('should transform custom functions', () => {
    const profileWithCustomFn: StoreMappingProfile = {
      name: 'Custom',
      requiredFields: ['externalId', 'price', 'name'],
      fields: {
        externalId: 'id',
        ...
        size: {
          path: 'size',
          transform: (val: string) => val ? val.replace('EU', '').trim() : undefined
        }
      }
    };
