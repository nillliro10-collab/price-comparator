...
import { calculateTextSimilarity } from './matching.score';

// In a real app, this would be a DB call `prisma.variant.findMany(...)`
// For the matching service logic, we'll assume we pass it the candidate variants
...
export class ProductMatchingService {
  ...
  public match(offer: NormalizedOffer, candidates: CanonicalVariantCandidate[]): MatchingResult {
    ...
    if (offer.mpn && offer.brand) {
      const mpnMatch = candidates.find(c => 
        c.mpn === offer.mpn && 
    ...
    let bestCandidate: CanonicalVariantCandidate | null = null;
    let highestScore = 0;
    ...
    for (const candidate of candidates) {
      ...
      let similarity = calculateTextSimilarity(offer.title, normalizedCandidateName);
    ...
    if (bestCandidate) {
      ...
      if (highestScore >= 0.90) confidence = 'HIGH';
      else if (highestScore >= 0.75) confidence = 'MEDIUM';
      ...
      if (highestScore > 0.65) { // Un umbral base para considerarlo candidato a revisión
        ...
        if (!sameSize) reasons.push(`Size mismatch: offer ${offer.size.sizeValue} vs canonical ${bestCandidate.sizeValue}`);
